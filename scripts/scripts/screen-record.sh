...
# Added check for wl-copy to ensure clipboard functionality works
if ! command -v wl-copy &> /dev/null; then
    notify-send -u critical "Screen Recording" "wl-clipboard is not installed"
    exit 1
fi
...
start_recording() {
    ...
    # Start wf-recorder without the --audio flag
    wf-recorder -g "$geometry" -f "$mp4file" > "$logfile" 2>&1 &
    recorder_pid=$!
...
stop_recording() {
    ...
    notify-send -u normal "Screen Recording" "Recording saved and ready to paste!\nLocation: $mp4file"
