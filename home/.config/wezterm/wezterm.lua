...
-- ── Window chrome ─────────────────────────────────────────────────────────────
-- Linux: "NONE" = no title bar / no resize border (fully hidden). Resize/move via Hyprland (e.g. Super+drag).
-- macOS: "RESIZE" keeps resizable edges without a title strip.
if wezterm.target_triple:find("linux") then
	config.font_size = 11 -- Hyprland fractional scale (raise if too small)
	config.window_decorations = "NONE"
else
	config.font_size = 16
	config.window_decorations = "RESIZE"
end
-- Avoid resize-to-preserve-cell-count when font/DPI changes; maximized windows can’t resize (WARN in overlay).
